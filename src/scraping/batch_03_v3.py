"""Batch 3 v3 — Final corrections: dedup, co-winners, name cleaning."""
...
WINNERS = {
    ...
    # KL25: determined from jury table
}
FINALISTS = {
...
def clean_name(name):
    """Remove Wikipedia reference markers from names."""
    name = re.sub(r'\s*\[\s*n[°o\s]*\d+\s*\]', '', str(name), flags=re.I)
...
def parse_name(raw):
    raw = clean(raw)
    # Strip "Returned to Game" before parsing
    raw = re.sub(r'\s*Returned\s+to\s+Game\s*', ' ', raw, flags=re.I).strip()
    m = re.match(r'^(.*?)\s+(\d{1,3})\s*,\s*(.*?)$', raw)
    if m: return clean_name(m.group(1).strip()), m.group(2).strip()
...
def extract_jury_winners(url):
    """Extract winners from jury vote table. Returns dict of {name: votes}."""
    ...
    for t in tables:
        ...
        if 'jury vote' not in h and 'jury' not in h: continue
        # Find the votes row (4-4-3 format)
        finalists = {}
        for row in rows:
            ...
            if re.match(r'^\d+-\d+-\d+$', row_text.strip()):
                # This is the votes row; the header row preceding it has candidate names
                pass
            # Look for the "Finalist" row with candidate names
            if 'finalist' in ' '.join(ths) or 'finalist' in ' '.join(tds):
                # Names are in subsequent cells
                names = [c for c in tds if c and c not in ('Votes','Vote','') and not re.match(r'^\d+',c)]
                # Find the row with votes
                pass
        
        # Actually, the jury table structure is:
        # Row 0: headers (Episode #, Finale)
        # Row 1: Day #, 40
        # Row 2: Finalist, Bastien, François, Géraldine
        # Row 3: Votes, 4-4-3
        names_row = None
        votes_row = None
        for row in rows:
            ...
            tds = [td.get_text(strip=True) for td in row.find_all("td")]
            if 'finalist' in ' '.join(ths):
                names_row = tds
            if 'votes' in ' '.join(ths) or re.match(r'^\d+-\d+-\d+$', ' '.join(tds)):
                votes_row = tds
        
        if names_row and votes_row:
            vote_str = ' '.join(votes_row)
            m = re.match(r'^(\d+)-(\d+)-(\d+)$', vote_str.strip())
            if m and len(names_row) >= 3:
                return {
                    names_row[0]: int(m.group(1)),
                    names_row[1]: int(m.group(2)),
                    names_row[2]: int(m.group(3)),
                }
    return None

def scrape_en_dedup(url):
    """Scrape English contestant table with proper 'Returned to Game' deduplication."""
    resp = requests.get(url, headers={"User-Agent":USER_AGENT}, timeout=30)
    soup = BeautifulSoup(resp.text, "lxml")
    tables = soup.find_all("table", class_="wikitable")
    ct = None
    ...
    for row in rows[1:]:
        ...
        is_return = "Returned to Game" in first or "Returned to Game" in finish
        raw.append({"name":name,"age":age,"finish":finish,"cells":cells,"is_return":is_return})
    
    # Dedup: when a candidate has a return marker row, skip it and mark the canonical entry
    # Strategy: group by name, keep only the last (non-return) occurrence
    # The canonical entry is the one WITHOUT "Returned to Game" in its row
    name_map = {}  # name -> {"canonical": row, "has_return": bool}
    for r in raw:
        n = r["name"]
        if n not in name_map:
            name_map[n] = {"canonical": r, "has_return": r["is_return"]}
        else:
            if not r["is_return"]:
                # This is the real entry (age, full details); use it
                name_map[n]["canonical"] = r
                name_map[n]["has_return"] = r["is_return"] or name_map[n]["has_return"]
    
    # Build canonical list in original table order
    seen = set()
    canonical = []
    for r in raw:
        n = r["name"]
        ...
        entry = name_map[n]
        c = entry["canonical"]
        c["returned"] = entry["has_return"] or r.get("returned", False)
        canonical.append(c)
    
    return canonical

def build_df(candidates, sid, sname, syear, surl, lang, jury_winners=None):
    ws = set(WINNERS.get(sid,[]))
    fs = set(FINALISTS.get(sid,[]))
    
    # If jury winners provided, use them for partial name matching
    if jury_winners and sid == "KL25":
        max_votes = max(jury_winners.values())
        ws_jury = {n for n, v in jury_winners.items() if v == max_votes}
        fs_jury = {n for n, v in jury_winners.items() if v < max_votes}
        # Apply to actual candidate names (match by prefix/substring)
        ws = set()
        fs = set()
        for c in candidates:
            cn = clean_name(c["name"])
            for jn in ws_jury:
                if jn.lower() in cn.lower() or cn.lower() in jn.lower():
                    ws.add(cn)
            for jn in fs_jury:
                if jn.lower() in cn.lower() or cn.lower() in jn.lower():
                    fs.add(cn)
...
def main():
    ...
    for sid, s in seasons.items():
        ...
        if s["lang"]=="en":
            ...
            # For KL25, extract jury winners since contestant table doesn't mark both
            if sid == "KL25":
                jury_winners = extract_jury_winners(s["url"])
                if jury_winners:
                    print(f"  Jury: {jury_winners}")
        else:
            ...
            for row in rows[1:]:
                ...
                candidates.append({"name":clean_name(clean(name)),"age":clean(age) if age else None,"finish":depart,"returned":False})
        ...
        df = build_df(candidates, sid, s["name"], s["year"], s["url"], s["lang"], jury_winners)
        ...
        print(f"  Winners: {list(w['candidate_name'])}, w={w['departure_type_normalized'].tolist()}, censored={w['censored_at_end'].tolist()}")
        all_dfs.append(df)
    ...
    for sid in sorted(batch.season_id.unique()):
        ...
        print(f"  {sid}: {n} candidates, winners={list(ws['candidate_name'])} ({ws['departure_type_normalized'].tolist()})")
